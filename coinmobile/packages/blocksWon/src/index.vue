<template>
    <div class="blocks-won bottom-20 bg-board" :class="{ 'not-index': !atIndex }">
        <div class="blocks-title flex jus-between">
            <span>{{ tr("title") }}</span>
            <client-only>
        <span
                class="see-more"
                v-show="isMobile && atIndex"
                @click="goTo('blocksWon')"
        >
          {{ tr("more") }}
        ...
        <div class="right-con flex">
            <div
                    class="latest-time font-14 font-400 flex align-center"
                    v-html="tr('latest', { time: formatLatestTime })"
            ></div>
        </div>
        ...
        <client-only>
            <div
                    class="see-more border flex align-center jus-center"
                    @click="goTo('blocksWon')"
                ...
                if (Array.isArray(topn_miners)) {
                    ...
                    tipsets.forEach((item, index) => {
                        ...
                        if (index == 0 && item.blocks.length > 0) {
                            ...
                            this.formatLatestTime = this.formatTime(
                                this.latestTime,
                                null,
                                false
                            )
                            this.setLatestBlockTime(
                                this.formatTime(this.latestTime, null, false)
                            )
                        }
                    })
