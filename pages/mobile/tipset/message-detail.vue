  ...
  <div class="message-detail bottom-10 bg-board radius">
    ...
    <cm-card v-if="consumeList!==null">
      ...
      <cm-cards
              ...
              :border="false"
              :style="index===consumeList.length-1?'':'border-bottom: 1px dashed var(--border-line)'"
      ></cm-cards>
    </cm-card>
    <cm-card>
      <div slot="header" class="clearfix">
...
<script>
export default {
  name: "CmMessageDetail",
  layout: 'mlayout',
  data(vm) {
    return {
      infoLabel: vm.$t('message.detail.infoview'),
      dataLabel: vm.$t('message.detail.overview'),
      otherLabel: vm.$t('message.detail.otherview'),
      transactionLabels:  vm.$t('message.detail.mbTransferLabels'),
      titles: vm.$t('message.detail.titles'),
      cid: "",
      loading: false,
      columns: [
        {
  ...
  computed:{
    columnsWithLabels(){
      let labels = this.transactionLabels
