<template>
  <div class="message-detail radius">
    <co-overdetail
      :dataList="dataList"
              ...
              render() {
                return (
                  <div class="height-link">
                    <nuxt-link to={`./tipset/chain?height=${sourceMap.heightNum}`}>{sourceMap.height}</nuxt-link>
                    <span> {exit_code == '0' || !exit_code ? ` ( ${sourceMap.confirm} ${confirm} )` : ''}</span>
                  </div>
                )
              }
            }
