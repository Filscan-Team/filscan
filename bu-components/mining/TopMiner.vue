<template>
  <div class="top-miner radius" v-loading="loading" element-loading-background="transparent">
    ...
    <div class="see-more content-bg-color" v-else>
      <co-view-more name="mining"  :switch-type="'1'"/>
    </div>
  </div>
</template>
<script>
    ...
    async getTopMiners() {
      ...
      try {
        ...
        if (res.data && Array.isArray(res.data)) {
          const maxPower = Math.max(...res.data.map((item) => item.quality_adj_power))
...
@mixin column {
  $width: 50 60 140 300 150 140 160 160 160 130 300;
